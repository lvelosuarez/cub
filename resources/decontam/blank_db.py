...
def _(MATE_SUFFIX_RE, UNINFORMATIVE_TAXIDS, pl):
    def _strip_mate_suffix(s: str) -> str:
        return MATE_SUFFIX_RE.sub("", s)

    def _parse_kmer_string(kmer_str: str) -> tuple[float, float, int, int]:
        if not kmer_str or kmer_str.strip() == "":
            return 0.0, 0.0, 0, 0
        total = 0; classified = 0; taxa_counts: dict[int, int] = {}
...
def _(
    ...
    def classify_read(df: pl.DataFrame, taxid_kingdom_df: pl.DataFrame) -> pl.DataFrame:
        df_k = df.join(
        ...
        return (
            df_k.with_columns(
                pl.when(k1_classified & (is_human | is_human_kingdom) & (pl.col("k1_kmer_hit_frac") >= HUMAN_KHF_HARD_CUTOFF) & ~has_breadth).then(pl.lit("HUMAN_CONFLICT"))
                  ...
                  .when(k1_classified & is_eukaryote & k1_eukaryote_quality & has_k2 & k2_eukaryote_quality).then(pl.lit("EUKARYOTE_CONFIRMED"))
                  .when(k1_classified & is_eukaryote & k1_eukaryote_quality & ~has_k2).then(pl.lit("EUKARYOTE_K1_ONLY"))
...
def _():
    ...
    COHORT = [
        ...
        {"sample_id": "KG_r21",    "folder": "250213_NB501647_0293_AH32GMBGYW",   "sample": "A1001_S1", "dna_ng": 0.95, "is_blank": False, "run": 21},
        {"sample_id": "EMB_r21",   "folder": "250213_NB501647_0293_AH32GMBGYW",   "sample": "A1002_S2", "dna_ng": 1.16, "is_blank": False, "run": 21},
        {"sample_id": "LA_r21",    "folder": "250213_NB501647_0293_AH32GMBGYW",   "sample": "A1003_S3", "dna_ng": 0.26, "is_blank": False, "run": 21},
        {"sample_id": "SJC_r21",   "folder": "250213_NB501647_0293_AH32GMBGYW",   "sample": "A1004_S4", "dna_ng": 0.11, "is_blank": False, "run": 21},
        {"sample_id": "BLANK_r21", "folder": "250213_NB501647_0293_AH32GMBGYW",   "sample": "A1005_S5", "dna_ng": 0.92, "is_blank": True,  "run": 21},
...
@app.cell
def _(P_EUPATH_INSPECT, P_KRAKEN2_NAMES, P_KRAKEN2_NODES, pl):
    def _(P_EUPATH_INSPECT, P_KRAKEN2_NAMES, P_KRAKEN2_NODES, pl):
        print("Loading taxonomy resources (once)...")

    # ── nodes.dmp → parent + rank dicts ───────────────────────────────────────
    # NOTE: no leading underscore — Marimo only exports names without _ prefix
    parent_g: dict[int, int] = {}
    rank_g:   dict[int, str] = {}
    ...
    # ── EuPathDB inspect.txt → k2_taxid → name DataFrame ─────────────────────
    _eupath_rows = []
    ...
    # ── taxmap (shared across all samples) ────────────────────────────────────
    taxmap_g = (
        pl.read_csv("/mnt/san/microbio/apps/cub/resources/bartlett_kraken2_index.tsv",
                    separator="\t")
...
def _(
    ...
    # ── superkingdom resolver (uses pre-loaded parent_g) ─────────────────────
    _SUPERKINGDOM_ROOTS = {
        2: "Bacteria", 2157: "Archaea", 2759: "Eukaryota",
        10239: "Viruses", 9606: "Human",
    }

    def _resolve_kingdom(taxid: int) -> str:
        current = taxid; visited: set[int] = set()
    ...
    # ── species finder (verbatim logic from summarise_sample) ─────────────────
    ...
    def _find_species(taxid: int) -> int:
        if taxid in _NO_COLLAPSE:
            return taxid
        if rank_g.get(taxid, "no rank") in _STOP_RANKS:
            return taxid
        current = taxid; visited: set[int] = set()
    ...
    # ── lineage builder (verbatim logic from summarise_sample) ────────────────
    _LINEAGE_RANKS = {"superkingdom", "phylum", "class", "order", "family", "genus", "species"}

    def _get_lineage(taxid: int) -> dict[str, int]:
    ...
    def run_blank(s: dict) -> pl.DataFrame:
        """
        Full pipeline for one blank sample dict from COHORT.
        Mirrors the cell-by-cell flow of the interactive notebook exactly.
        Returns sample_row (same schema as the interactive notebook output).
        """
        sid = s["sample_id"]

        # ── Step 1: load K1 + K2 (identical to interactive cell) ──────────────
        k1 = load_kraken2_per_read(s["p_k1"], db_tag="pluspf")
        k2 = load_kraken2_per_read(s["p_k2"], db_tag="eupath")

        # ── Step 2: kingdom resolution (identical to interactive cell) ─────────
        _all_taxids = (
            pl.concat([k1.select("taxid"), k2.select("taxid")])
            .unique().filter(pl.col("taxid") > 1)["taxid"].to_list()
        )
        ...
        # ── Step 3: BAM scan + breadth + taxmap (identical to interactive cell) ─
        rows: list[dict] = []
        ...
        breadth_df = pl.DataFrame(
            [{"bam_rname": ref, "ref_length": ref_lengths[ref],
              ...
              "breadth_fraction":  (sum(1 for h in hits if h >= MIN_DEPTH) * WINDOW_SIZE) / ref_lengths[ref]
                                   if ref_lengths[ref] > 0 else 0.0}
        ...
        # MAPQ filter (identical to interactive cell)
        high_confidence = (
        ...
        # ── Step 4: merge K1 + K2 + BAM (identical to interactive cell) ────────
        def _prefix(df: pl.DataFrame, tag: str) -> pl.DataFrame:
            return df.rename({c: f"{tag}_{c}" for c in df.columns if c != "read_id"})

        k1p = _prefix(k1, "k1"); k2p = _prefix(k2, "k2")
        merged_k1 = (
            k1p.filter(pl.col("k1_status") == "C")
            .join(k2p,    on="read_id", how="left")
            .join(bam_df, on="read_id", how="left")
        )
        merged_bam_only = (
            bam_df
            .join(merged_k1.select("read_id"), on="read_id", how="anti")
            .join(k1p, on="read_id", how="left")
            .join(k2p, on="read_id", how="left")
        )
        merged = pl.concat([merged_k1, merged_bam_only], how="diagonal_relaxed")

        # ── Step 5: classify_read (calls the function from Cell 3) ─────────────
        passed = classify_read(merged, taxid_kingdom_df)

        # ── Step 6: human filter (identical to interactive cell) ────────────────
        nonhuman = passed.filter(~pl.col("read_call").is_in(list(HUMAN_CALLS)))

        # ── Step 7: name resolution (uses pre-loaded dicts) ────────────────────
        _k1_taxids = set(nonhuman["k1_taxid"].drop_nulls().unique().to_list())
        taxid_name_df = pl.DataFrame(
            [{"k1_taxid": t, "k1_sci_name": name_lookup_g.get(t, str(t))}
             for t in _k1_taxids],
            schema={"k1_taxid": pl.Int64, "k1_sci_name": pl.Utf8},
        )

        # ── Step 8: summary aggregation (identical to interactive cell) ─────────
        summary = (
            nonhuman
            .join(taxid_name_df,  on="k1_taxid", how="left")
            .join(k2_name_df_g,  on="k2_taxid", how="left")
            .with_columns([
                pl.when(pl.col("read_call") == "EUKARYOTE_CONFIRMED").then(pl.col("k2_taxid"))
                  .when(pl.col("read_call") == "BAM_ONLY").then(pl.col("bam_taxid_species"))
                  .otherwise(pl.col("k1_taxid")).alias("display_taxid"),
                pl.when(pl.col("read_call") == "EUKARYOTE_CONFIRMED").then(pl.col("k2_sci_name"))
                  .when(pl.col("read_call") == "BAM_ONLY").then(pl.col("taxon_name"))
                  .otherwise(pl.col("k1_sci_name")).alias("display_name"),
            ])
        ...
        # ── Step 9: summarise_sample (verbatim from main notebook) ──────────────
        # Fix 1: drop Unknown kingdom
        ...
        sc = (
            ...
            .agg([
                ...
                pl.when(pl.col("read_call").is_in(["KRAKEN_CONFIRMED", "EUKARYOTE_CONFIRMED"])).then(pl.col("n_reads")).otherwise(0).sum().alias("n_reads_tier1"),
                pl.when(pl.col("read_call").is_in(["BAM_K1_LOW_CONF", "BACTERIA_K1_ONLY", "EUKARYOTE_K1_ONLY", "VIRUS_K1_ONLY", "BAM_ONLY"])).then(pl.col("n_reads")).otherwise(0).sum().alias("n_reads_tier2"),
                pl.when(pl.col("read_call").is_in(["BACTERIA_LOW_CONF", "HUMAN_KMER_BROAD_COVERAGE"])).then(pl.col("n_reads")).otherwise(0).sum().alias("n_reads_tier3"),
            ...
            .with_columns([
                ...
                pl.when(pl.col("group_taxid").map_elements(lambda t: rank_g.get(t, "no rank") == "genus", return_dtype=pl.Boolean))
                  .then(pl.lit("genus"))
                  .otherwise(pl.col("group_taxid").map_elements(lambda t: rank_g.get(t, "no rank"), return_dtype=pl.Utf8))
                  .alias("tax_rank"),
            ])
            # Fix 2: zero out breadth for tier4
            .with_columns([
        ...
        # lineage annotation (uses pre-loaded name_lookup_g)
        _lin_rows = []
        ...
        # Fix 3: minimum read floor + join lineage
        return (
...
def _(BLANKS, P_OUTPUT_DIR, pl, run_blank):
    """
    Sequential loop over all blanks. Each blank takes ~60-90 s.
    Expected total: ~20-30 minutes.
    Raw results saved to blank_cohort_raw.parquet after each blank
    so you can recover partial results on interrupt.
    """
    _results = []
    _errors  = []
...
@app.cell
def _(BLANKS, blank_cohort, pl):
    # ── library size normalization ───────────────────────────────
    # Total reads = all reads in K1 file (classified + unclassified)
    # This is the true sequencing depth denominator
    def _count_total_reads_k1(path: str) -> int:
        """Count all lines in a .krk file = total reads sequenced."""
        n = 0
        with open(path) as fh:
            for line in fh:
                if line.strip():
    ...
    # get library sizes for all blanks
    lib_sizes = []
    for s in BLANKS:
        total = _count_total_reads_k1(s["p_k1"])
        lib_sizes.append({
    ...
    # ── Normalise blank_cohort to RPM ─────────────────────────────────────────────
    blank_cohort_norm = (
        blank_cohort
        .join(lib_df.select(["sample_id", "total_reads"]), on="sample_id", how="left")
        .with_columns([
            (pl.col("n_reads_total") / pl.col("total_reads") * 1_000_000)
            .alias("rpm"),
        ])
    )
...
def _(blank_cohort, blank_cohort_norm, pl):
    """
    Aggregates all blank results into a per-organism contamination profile.

    Classification logic:
      UNIVERSAL_KIT : prevalence >= 75% (present in ≥75% of all blanks)
      RUN_BATCH     : reads in run28 blank > 10× mean across other blanks
                      → confirmed lot contamination event
      REAGENT       : present in ≥3 blanks but not universal
      SPORADIC      : present in 1-2 blanks only

    All categories get a contam_flag column. Nothing is deleted from the
    patient cohort — organisms are annotated, not dropped.
    """
    _N = blank_cohort["sample_id"].n_unique()

    # per-blank reads for run28 (special column for batch effect detection)
    _run28_reads = (
    ...
    contaminant_db = (
        ...
        .with_columns(
            pl.when(pl.col("prevalence") >= 0.75)
              ...
              # RUN_BATCH: reads in run28 blank > 10× mean across all blanks
              .when(
    ...
    # ── Add to contaminant_db build cell ─────────────────────────────────────────
    contaminant_db_final = (
        contaminant_db
        # join lib_df to get total_reads per blank
        .join(
            blank_cohort_norm
        ...
        # flag the lot-contaminated runs per organism
        .with_columns(
...
def _(blank_cohort):
    """
    For each run × organism, records the blank read counts.
    Used during patient analysis as:
      contamination_score = patient_reads / (patient_reads + blank_reads)
    Flag patient calls where score < 0.9.
    """
    run_blank_ref = (
        blank_cohort
        .select([
...
def _(P_OUTPUT_DIR, blank_cohort, contaminant_db_final, pl, run_blank_ref):
    """
    Writes three files consumed by the main patient analysis notebook:

    blank_cohort_raw.parquet  — already checkpointed during the loop (overwritten here)
    contaminant_db.parquet    — static contaminant database (primary decontam input)
    contaminant_db.tsv        — human-readable version for manual review
    run_blank_ref.parquet     — per-run blank reads for ratio filter
    """
    ...
    # ── flatten list columns before CSV export ────────────────────────────────────
    (
        contaminant_db_final
        .with_columns(
            pl.col("runs_present")
